...
func (l *MyList) ReverseIterator() *Iterator {
	return &Iterator{data: l.data, index: len(l.data)}
}
...
func (i *Iterator) ReverseNext() int {
	if i.index == 0 {
		panic(fmt.Errorf("No more elements"))
	}
	i.index--
...
func (i *Iterator) CyclicNext() int {
...
func main() {
	...
	for scanner.Scan() {
		...
		case "reverse":
			 fmt.Print("[ ")
			it := mylist.ReverseIterator()
			for it.index > 0 {
				fmt.Printf("%v ", it.ReverseNext())
			}
			fmt.Println("]")
		case "cyclic":
